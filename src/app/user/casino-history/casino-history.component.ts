...
@Component({
  ...
  styleUrls: ['./casino-history.component.css']
})
export class CasinoHistoryComponent implements OnInit {
  ...
  getdTicketsPlayer() {
    ...
    this.userServ.getTransaction(this.idUser).subscribe((res: any) => {
  ...
  getdTicketsPlayerPage(page) {
    ...
    this.page = page;
    this.userServ.getTransictionsByPage(this.idUser, page).subscribe((res: any) => {
      this.lengthdata = res.data.items.length;
      if (res) {
